...
(function() {
  const html = `
<nav id="vs-nav">
  <a class="nav-logo" href="/">Vegas <span>Sidekick</span></a>
  <ul class="nav-links">
    <li><a href="/shows/comedy/">Comedy</a></li>
    <li><a href="/shows/magic/">Magic</a></li>
    <li><a href="/shows/cirque/">Cirque</a></li>
    <li><a href="/shows/production/">Production</a></li>
    <li><a href="/search/" class="nav-search-link">🔍 Search</a></li>
...
<div class="nav-mobile-drawer" id="vsMobileDrawer">
  ...
  <a href="/shows/cirque/">Cirque</a>
  <a href="/shows/production/">Production</a>
  <a href="/search/">🔍 Search Shows</a>
...
<div class="nav-overlay" id="vsNavOverlay" onclick="vsToggleMenu()"></div>`;
  ...
  const styles = `
<style>
#vs-nav { position: fixed; top: 0; left: 0; right: 0; z-index: 200; display: flex; align-items: center; justify-content: space-between; padding: 14px 40px; background: rgba(10,22,40,0.95); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(26,127,232,0.15); }
.nav-logo { font-family: 'Bebas Neue', sans-serif; font-size: 1.6rem; color: #2E9BFF; letter-spacing: 2px; text-decoration: none; }
...
.nav-search-link { color: #A8B4C4 !important; }
.nav-cta { background: #FF6B00 !important; color: #0A1628 !important; padding: 8px 20px; border-radius: 4px; }
.nav-mobile-menu { display: none; background: none; border: none; color: #D4DCE8; font-size: 1.4rem; cursor: pointer; padding: 4px 8px; }
.nav-mobile-drawer { display: none; position: fixed; top: 0; right: -280px; width: 280px; height: 100vh; background: #0D1F3C; z-index: 300; flex-direction: column; padding: 80px 32px 32px; gap: 8px; transition: right 0.3s ease; border-left: 1px solid rgba(26,127,232,0.2); }
